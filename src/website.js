...
const TintWrapWebsite = () => {
  ...
  // Handle router state from Navbar: { openQuote: true } or { scrollTo: "contact" | "services" | "top" }
  useEffect(() => {
    ...
    if (state.scrollTo === 'contact') {
      ...
      handled = true;
    } else if (state.scrollTo === 'services') {
      scrollToSection(servicesRef);
      handled = true;
    } else if (state.scrollTo === 'top') {
      window.scrollTo({ top: 0, behavior: 'smooth' });
      handled = true;
    }

    // Clear state so it doesn't retrigger
    if (handled) {
  ...
  const services = [
    {
      ...
      description:
        'We provide top-quality car window tinting and wrapping services at the most competitive prices, backed by a service guarantee. Our tinting enhances both driver safety and passenger comfort. We use premium window tint film that effectively reduces heat and blocks up to 99% of harmful UV rays.',
    ...
    },
    {
      title: 'Vehicle Wraps',
      description:
        "Looking for car wrapping services in Brisbane? You're in the right place! Galaxy Tint and Wrap offers professional vehicle wrapping services in the area. A vehicle wrap is a vinyl covering applied to the car's paint, giving it a fresh new look. It's easy to apply and remove, so if you want a new color or style, you can simply replace the wrap.",
    ...
    },
    {
      ...
      description:
        'Ceramic coating is a long-lasting protective layer applied to a vehicle’s exterior that shields it from UV rays, dirt, water, and everyday wear. It enhances the paint’s gloss and depth while making the surface easier to clean, helping maintain a like-new appearance for longer.',
